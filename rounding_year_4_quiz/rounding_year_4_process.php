...
<?php
    ...
    if($_POST) {
        ...
        //get correct answer
        $query = "SELECT *  FROM `choices` WHERE questions_number = $number AND topic = 'rounding_year_4' AND is_correct = 1";
        ...
        if($number == $total){
            header("location: rounding_year_4_final.php?=finished");
            exit();
        } else {
            header("location: rounding_year_4_quiz.php?n=".$next);
        }
    }